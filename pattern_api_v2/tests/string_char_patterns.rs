...
macro_rules! searcher_test {
    ...
    ($name:ident, $p:expr, $h:expr, double, is valid) => {
        ...
        mod $name {
            use SearchResult::{Match, Reject};
            use super::{cmp_search_to_vec, compare};
...
fn cmp_search_to_vec<'a, P, F>(rev: bool,
                               pat: F,
                               haystack: &'a str,
...
fn is_malformed(v: &[SearchResult], haystack: &str) -> bool {
    let mut found = false;
...
fn compare(left: &[SearchResult], right: &[SearchResult], haystack: &str) {
